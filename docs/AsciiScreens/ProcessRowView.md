...
menu with Start, Stop, Restart, Kill, Open log, Clear log, and Copy ID for agent.
...
- Context menu: Start / Stop / Restart / Kill / Open log / Clear log / Copy ID
  for agent, wired to `ManagedProcess.start()/stop()/restart()/kill()`,
  `onOpenLog`, `onClearLog`, and `onCopyId`. "Clear log" empties the process's
  output buffer (`ManagedProcess.clearLog()`) while it keeps running, so an open
  log pane redraws from empty. "Copy ID for agent" copies the process's `ManagedProcessID`
